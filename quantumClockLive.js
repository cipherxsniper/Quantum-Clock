// quantumClockLive.js
// Thomas Lee Harvey Quantum Clock Live
...
// -------------------- Logging First Run --------------------
const LOG_FILE = "./quantumClockLog.json";
function logFirstRun() {
  const now = new Date();
  const { psiF } = calculatePsiF(C, 500, V, PHI);
  if (!fs.existsSync(LOG_FILE)) {
    fs.writeFileSync(
      LOG_FILE,
      JSON.stringify([{ timestamp: now.toISOString(), psiF }], null, 2)
    );
  }
}
...
function runLiveClock() {
  ...
  setInterval(() => {
    ...
    const secAngle = ((sec % 60) / SECOND_COUNT) * 2 * Math.PI - Math.PI / 2;
    ...
    sec++;
  }, 200); // updates every 0.2s for smooth sub-second movement
}

// -------------------- START --------------------
console.log("Starting Thomas Lee Harvey Quantum Clock Live...");
runLiveClock();
